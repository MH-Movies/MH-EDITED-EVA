...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"🗓 PAGE 1/{round(int(total_results)/10)} ",callback_data="pages"), InlineKeyboardButton(text="NEXT PAGE ➾",callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
...
async def advantage_spell_chok(msg):
    ...
    btn.append([InlineKeyboardButton(text="<b>Couldn't Find This Movie.Try Again ഈ സിനിമയുടെ ഒറിജിനൽ പേര് ഗൂഗിളിൽ പോയി കണ്ടെത്തി അതുപോലെ ഇവിടെ കൊടുക്കുക 🥺</b>",
